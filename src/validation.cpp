                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s XCOIN additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if(nHeight<131)
      {
	return CAmount(10256 * COIN);
      }
    
    CAmount nSubsidy = 256 * COIN;
    // Subsidy is cut in half every 100,000 blocks which will occur approximately every year.
    nSubsidy >>= halvings;
    return nSubsidy;
}
